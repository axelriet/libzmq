...
int zmq::norm_engine_t::init (_In_z_ const char *network_, bool send, bool recv)
{
    ...
    unsigned short portNumber = (unsigned short) p;
    ...
    if (!NormIsUnicastAddress (addr)) {
        ...
        NormSetTTL (norm_session, options.multicast_hops <= 255
                                    ? (unsigned char) options.multicast_hops
                                    : 255);
    ...
    if (send) {
        ...
        if (!NormStartSender (norm_session, instanceId,
                              ...
                              (INT16) options.norm_segment_size, blocksize,
                              numparity)) {
            //
            // errno set by whatever failed
            //
...
void zmq::norm_engine_t::terminate ()
{
    ...
    shutdown ();
    delete this;
} // end zmq::norm_engine_t::terminate()
