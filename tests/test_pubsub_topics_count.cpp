...
void _test_independent_topic_prefixes (const char *endpoint_, bool multicast_ = false)
{
    //  Create a publisher
...
void _test_nested_topic_prefixes (const char *endpoint_, bool multicast_ = false)
{
    //  Create a publisher
    void *publisher = test_context_socket (ZMQ_PUB);
...
void test_independent_and_nested_topic_prefixes_xnorm ()
{
#if defined ZMQ_HAVE_NORM
    _test_independent_topic_prefixes (
      "xnorm://" PRIVATE_EXPERIMENT_MULTICAST ":8310", true);
    _test_nested_topic_prefixes (
      "xnorm://" PRIVATE_EXPERIMENT_MULTICAST ":8311", true);
#else
    TEST_IGNORE_MESSAGE ("libzmq without NORM, ignoring test.");
#endif
}
...
int ZMQ_CDECL main ()
{
    ...
    RUN_TEST (test_independent_and_nested_topic_prefixes_xnorm);
